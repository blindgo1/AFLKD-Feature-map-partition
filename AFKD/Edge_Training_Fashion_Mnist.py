...
transform = transforms.Compose([
    transforms.Resize(32), # 将图像大小调整为(224, 224)
    transforms.ToTensor(),  # 将图像转换为PyTorch张量
    transforms.Normalize((0.5,), (0.5,))  # 对图像进行归一化
])

# MNIST
training_set = FashionMNIST(root='./data', train=True, download=True, transform=transform)
test_set = FashionMNIST(root='./data', train=False, download=True, transform=transform)

# 限制训练集数据量到前 2000 张
subset_indices = list(range(1500))  # 创建索引列表
...
state_dict = torch.load('pth/model_cloud_Fashion_Mnist.pth')
#加载教师模型
...
# 训练结束，保存模型参数
torch.save(student_model.state_dict(), 'pth/model_Edge_Mnist.pth')

# 加载训练好的模型参数
student_model.load_state_dict(torch.load('pth/model_Edge_Mnist.pth'))
