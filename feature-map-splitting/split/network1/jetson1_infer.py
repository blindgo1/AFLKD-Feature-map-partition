...
from model import VGG16Small_part1,create_model_part2_instance
...
def jetson1_inference(rank ):
    ...
    model_part2_rref = rpc.remote(f"raspberrypi8", create_model_part2_instance)
    ...
    with torch.no_grad():
        # for images, labels in train_loader:
            ...
            output = rpc.rpc_sync(f"raspberrypi8", fc_training, args=(model_part2_rref, intermediate_output.to('cpu'), 0)) # 将中间结果发送到raspberry上进行调用
            end_time = time.time()
            print(output.shape)
