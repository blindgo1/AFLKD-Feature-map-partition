...
def fc_training(model_part2_rref, input_tensor,rank):
    #现将全连接层前各设备的分块计算结果存储
    storage.store_data(f'fc_{rank}', input_tensor)
    # 当两个节点都将最后的卷积结果传输到storage中时
    while ((storage.retrieve_data(f'fc_0') is None) or (storage.retrieve_data(f'fc_1') is None)):
        pass
    # 拼接最后结果
    tensor = torch.cat(
        (storage.retrieve_data(f'fc_0'), storage.retrieve_data(f'fc_1')), 2)
    print('fc:',tensor.shape)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_part2 = model_part2_rref.local_value().to(device)
    output = model_part2(tensor.to(device))
    return output
