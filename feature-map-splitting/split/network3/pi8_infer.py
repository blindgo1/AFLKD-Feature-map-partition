...
def training8(model_part3_rref, input_tensor,rank):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model_part3_rref.to_here()
...
def init_rpc(rank, world_size):
    ...
    rpc.init_rpc("raspberrypi10", rank=rank, world_size=world_size, rpc_backend_options=options)
