...
def jetson1_inference():
    ...
    model_part4_rref = rpc.remote(f"raspberrypi7", create_model_part4_instance)
    ...
    intermediate_output3 = rpc.rpc_sync(f"raspberrypi6", training6,
                          args=(model_part3_rref, intermediate_output2,))
    output = rpc.rpc_sync(f"raspberrypi7", training7,
                          args=(model_part4_rref, intermediate_output3,))
    print(output.shape)
    end_time = time.time()
