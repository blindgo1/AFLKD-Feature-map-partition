...
class VGG16Small_part3(nn.Module):
    def __init__(self):
        ...
        # Conv Block 2
        self.conv6 = nn.Conv2d(48, 80, kernel_size=3, padding=0)
        self.relu6 = nn.ReLU(inplace=True)
        self.conv7 = nn.Conv2d(80, 80, kernel_size=3, padding=0)
        self.relu7 = nn.ReLU(inplace=True)
        self.conv8 = nn.Conv2d(80, 80, kernel_size=3, padding=0)
        self.relu8 = nn.ReLU(inplace=True)
        self.conv9 = nn.Conv2d(80, 80, kernel_size=3, padding=0)
        self.relu9 = nn.ReLU(inplace=True)
        self.conv10 = nn.Conv2d(80, 80, kernel_size=3, padding=0)
        self.relu10 = nn.ReLU(inplace=True)
        self.maxpool3 = nn.MaxPool2d(kernel_size=2, stride=2)

    def forward(self, x, rank):
        # i为卷积层的层数，第二部分卷积层从conv6开始
        i = 5
        for name, layer in self.named_children():
            #start_time = time.time()
            # 判断如果是卷积层
