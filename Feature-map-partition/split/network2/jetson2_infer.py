...
def jetson1_inference(rank):
    ...
    with torch.no_grad():
        ...
        intermediate_output2 = rpc.rpc_sync(f"raspberrypi6", training6, args=(
        model_part2_rref, intermediate_output,3))  # 将中间结果发送到raspberry上进行调用
        print(intermediate_output2.shape)
        output = rpc.rpc_sync(f"raspberrypi8", fc_training, args=(model_part3_rref, intermediate_output2, 3,))
        print(output.shape)
        end_time = time.time()
